...
class Client(object):
    ...
    def build_msg(self, config={}):
        config.setdefault('server_name', self.name)
        config.setdefault('project_id', self.project_id)
        config.setdefault('access_token', self.access_token)

        api_path = config.get(
                    'api_path',
                    defaults.ERROR_API_PATH.format(config['project_id'])
                )
        ...
        config.setdefault(
                    'servers',
                    [server + api_path for server in self.servers]
                )
    ...
    def capture(self, event_type, data=None, date=None, api_path=None,
        ...
        """
        Captures and processes an event and pipes it off to OpbeatClient.send.
        ...
        # kwargs = self.build_msg(**kwargs)
        # data = self.build_msg(**data)
    ...
    def send(self, message, project_id=None, access_token=None,
                   auth_header=None, servers=None):
        """
        ...
        try:
            return self.send_encoded(encoded_message,
    ...
    def send_encoded(self, message, project_id, access_token,
                           auth_header=None, servers=None):
        """
